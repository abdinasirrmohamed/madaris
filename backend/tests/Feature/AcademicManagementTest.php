...
class AcademicManagementTest extends TestCase
{
    ...
    public function test_academic_crud_is_tenant_scoped():void
    {
        ...
        $created=$this->postJson('/api/v1/academic/levels',['Name'=>'Hifdhi One','Code'=>'H1','SequenceNo'=>1,'MinimumPromotionScore'=>60,'Status'=>'Active'])->assertCreated()->json('data');
        $this->getJson('/api/v1/academic/levels')->assertOk()->assertJsonFragment(['Code'=>'H1'])->assertJsonMissing(['Code'=>'F']);
        $this->putJson('/api/v1/academic/levels/'.$created['LevelId'],['Name'=>'Hifdhi One','Code'=>'H1','SequenceNo'=>2,'MinimumPromotionScore'=>65,'Status'=>'Active'])->assertOk()->assertJsonPath('data.SequenceNo',2);
