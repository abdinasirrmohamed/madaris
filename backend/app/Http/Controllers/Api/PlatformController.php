...
class PlatformController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $result = DB::transaction(function () use ($data) {
            ...
            $base = $slug ?: 'school'; $suffix = 2;
            while (DB::table('Tenants')->where('Slug', $slug)->exists()) $slug = $base.'-'.$suffix++;
            ...
            foreach (DB::table('Permissions')->pluck('PermissionId') as $permissionId) DB::table('RolePermissions')->insert(['RoleId'=>$roleId,'PermissionId'=>$permissionId]);
    ...
    public function status(Request $request, int $tenant)
    {
        ...
        if ($data['Status']==='Suspended') User::where('TenantId',$tenant)->each(fn ($user) => $user->tokens()->delete());
